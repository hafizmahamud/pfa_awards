...
function filterPosts() {
    Inertia.get('/admin/news-announcements', {
        search: search.value,
...
function destroy(id) {
    if (confirm("Are you sure you want to Delete")) {
        Inertia.delete(route('dashboard-posts.destroy', id));
    }
}
</script>

<template>
    <AppLayout title="Add or Edit News and Announcements">
        <template #header>
            <AddOnHeader prefix="admin" title="Add or Edit News and Announcements" selected="admin.news_announcements"
                @jump-to="jumpTo">
            </AddOnHeader>
        </template>
                    ...
                    <div class="row">
                        <div class="col-12 mb-4">
                                ...
                                <table class="table">
                                    ...
                                    <tbody>
                                        <tr v-for="post in posts.data" :key="post.id">
                                            ...
                                            <td>{{ post.status }}</td>
                                            <td>
                                                <Link :href="post.edit_url" id="edit" class="btn-none">
                                                Edit</Link>
                                                <Link as="button" id="edit" class="btn-none" @click="destroy(post.id)">
                                                Delete</Link>
                                            </td>
                                        </tr>
                                    </tbody>
                            ...
                            <div class="text-end mt-4">
                                <Link as="button" :href="route('posts.create')" id="new-post" class="btn-primary">New
                                post</Link>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="row align-items-center g-0 mb-5">
                                <div class="col-12 col-md-4 mb-3 d-flex align-items-center">
                                    <div class="d-flex w-100 align-items-center me-4">
                                        <label class="me-3 w-100" for="tag">Result per page</label>
                                ...
                                <div class="col-12 col-md-8">
                                    <nav aria-label="Page navigation">
                                        <ul
                                            ...
                                            <PaginationLink v-for="link in props.posts.links" :href="link.url"
