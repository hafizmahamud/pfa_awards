...
function filterPosts() {
    Inertia.get(route('admin.documents'), {
        search: search.value,
...
function destroy(id) {
    if (confirm("Are you sure you want to Delete")) {
        Inertia.delete(route('document-collections.destroy', id));
    }
}
</script>

<template>
    <AppLayout title="Add or Edit Posts">
        <template #header>
            <AddOnHeader prefix="admin" title="Add or Edit Document Collections" selected="admin.documents"
                @jump-to="jumpTo">
            </AddOnHeader>
        </template>
                        ...
                        <div class="col-12 mb-4">
                            <div class="table-responsive  add-edit border border-color-secondary">
                                <table class="table">
                                    ...
                                    <tbody>
                                        <tr v-for="documentCollection in documentCollections.data"
                                            :key="documentCollection.id">
                                            <td>{{ documentCollection.updated_at }}</td>
                                            <td>{{ documentCollection.title }}</td>
                                            <td>
                                                <Link :href="documentCollection.edit_url" id="edit" class="btn-none">
                                                Edit</Link>
                                                <Link as="button" id="edit" class="btn-none"
                                                    @click="destroy(documentCollection.id)">
                                                Delete</Link>
                                            </td>
                                        </tr>
                                    </tbody>
                            ...
                            <div class="text-end mt-4">
                                <Link as="button" id="new-post" class="btn-primary"
                                    :href="route('document-collections.create')">
                                New Document
                                Collection</Link>
                            </div>
                        </div>
                                ...
                                <div class="col-12 col-md-8">
                                    <nav aria-label="Page navigation">
                                        <ul
                                            ...
                                            <PaginationLink v-for="link in documentCollections.links" :href="link.url"
